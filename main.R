...
clinical <- read.delim("./survival/clinical.cart.2024-07-22/clinical.tsv")
x <- c("case_id","case_submitter_id",'vital_status','days_to_last_follow_up')
clinical <- clinical[,x]
clinical <- clinical %>% distinct()

connect <- read.delim("./survival/gdc_sample_sheet.2024-07-22.tsv")

clinical_con <- merge(clinical, connect, by.x = "case_submitter_id", by.y = "Case.ID")

value <- {}
for(n in c(1:374)){
  file <- file.path("./survival/gdc_download_20240722_124106.302821/",clinical_con$File.ID[n],clinical_con$File.Name[n])
